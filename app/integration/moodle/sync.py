...
Run as a one-shot command from a container:

    python -m app.integration.moodle.sync

Not scheduled, not triggered by the API, not run on API startup — see
the README's "no scheduling yet" section. `main()` is the only place
that reads MOODLE_DB_URL/DATABASE_URL from the environment; everything
else takes engines as explicit arguments, so the pipeline itself never
depends on process-wide configuration and is easy to test.
"""
