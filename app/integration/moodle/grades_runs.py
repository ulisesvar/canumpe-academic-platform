"""integration.sync_runs / integration.sync_state bookkeeping for the
Moodle grades pipeline.

Same split as app.integration.moodle.runs: start_sync_run and
mark_sync_run_failed each open their own short transaction (bookkeeping
lives outside the academic transaction's atomicity boundary, so a
RUNNING row can always be flipped to FAILED); mark_sync_run_success and
advance_sync_state take an open connection and must run inside the same
transaction as the canonical merge. A separate module (rather than
reusing app.integration.moodle.runs) exists only because grades_merge's
MergeCounters carries rows_skipped, which the primary Moodle sync's
counters don't.
"""
...
from app.integration.moodle.grades_merge import MergeCounters
