"""Student-existence semantics shared by every Phase 5 endpoint: unknown
student_id -> 404 with a consistent body; a real student with no related
data -> 200 with empty/zeroed data, never an error.
"""
...
def test_existing_student_resolves(client: TestClient, db_engine: Engine) -> None:
...
def test_unknown_student_returns_404(client: TestClient) -> None:
    response = client.get(f"/students/{UNKNOWN_STUDENT_ID}/summary")

    assert response.status_code == 404
    assert response.json() == {"detail": "Student not found"}


def test_unknown_student_returns_404_on_every_endpoint(client: TestClient) -> None:
    for path in ("courses", "attendance", "grades", "summary"):
        response = client.get(f"/students/{UNKNOWN_STUDENT_ID}/{path}")
...
def test_valid_student_with_no_related_data_returns_200_not_an_error(
    ...
    for path in ("courses", "attendance", "grades", "summary"):
        response = client.get(f"/students/{student_id}/{path}")
